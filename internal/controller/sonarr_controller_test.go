...
var _ = Describe("Sonarr Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			err := k8sClient.Get(ctx, typeNamespacedName, sonarr)
			if err != nil && errors.IsNotFound(err) {
				resource := &feddemadevv1alpha1.Sonarr{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Sonarr")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &SonarrReconciler{
				Client: k8sClient,
